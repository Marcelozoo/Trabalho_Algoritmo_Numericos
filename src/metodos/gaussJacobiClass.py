...
class MetodoGaussJacobi:
    ...
    def calcula_sistema_linear(
        ...
        vetor_x = self.__realiza_substituicao(
    ...
    def __pega_pivo(self, matriz_A, ordem_matriz, vetor_b) -> None:
        """
        Essa função percorre a matriz_A e passa os pivos para a função self.pivoteai_matriz_A.
    ...
    def __realiza_substituicao(
        ...
        return self.__criterio_de_parada(vetor_x_novo, vetor_x, precisao)

    def __criterio_de_parada(
        self, vetor_x_novo: List[float], vetor_x: List[float], precisao: float
    ) -> None:
        """
        Essa função defini a precisão alcançada pelo metodo até entao e verifica se a precisão alcançada é
        ...
        for i in range(len(vetor_x)):
            with warnings.catch_warnings(record=True) as w:
                valores.append((vetor_x_novo[i] - vetor_x[i]))
    ...
    def get_converge(self) -> bool:
        """
        Essa função retorna se ocorreu convergência ou não dado um sistema que foi passado.Ela é usado
        no diretório de tests e na main.py
    ...
    def imprimir_resultados(self, qtd_sistemas: int) -> None:
        """
        Apenas imprimi os resultados obtidos pelo método gauss-jacobi.

        :return - vazio

        """

        print("Resultados dos sistemas resolvidos pelo Metodo de Gauss-Jacobi:")
